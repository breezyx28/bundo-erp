...
class Header extends Component
{
    ...
    {
        return <<<'HTML'
                <div id="{{ $anchor }}" {{ $attributes->class(['mb-6', 'scroll-mt-6' => $withAnchor]) }}>
                    <div class="flex flex-wrap gap-5 justify-between items-center">
                        <div>
                            {!! "<{$titleTag}" !!} @class(["flex items-center gap-2 text-xl font-semibold tracking-tight", is_string($title) ? '' : $title?->attributes->get('class') ]) >
                                @if($withAnchor)
                                    <a href="#{{ $anchor }}">
                                @endif
                            ...
                            @if($subtitle)
                                <div @class(["mt-1 text-sm text-base-content/60", is_string($subtitle) ? '' : $subtitle?->attributes->get('class') ]) >
                                    {{ $subtitle }}
                                </div>
                            @endif
                        </div>
