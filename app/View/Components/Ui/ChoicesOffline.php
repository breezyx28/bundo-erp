        ...
        return <<<'HTML'
                <div x-data="{ focused: false, selection: @entangle($attributes->wire('model')) }">
                        ...
                        x-data="{
                            ...
                            init() {
                                ...
                                document.addEventListener('livewire:navigating', () => {
                                    let elements = document.querySelectorAll('.ui-choices-element');
                    ...
                    >
                        <fieldset class="fieldset py-0">
                            ...
                            @if($label && !$inline)
                                <legend class="fieldset-legend mb-0.5">
                                    ...
                                    {{-- INPUT POPOVER --}}
                                    @if($popover)
                                        <x-ui-popover offset="5" position="top-start">
                                            <x-slot:trigger class="{{ $popoverTriggerClass }}">
                                                <x-ui-icon :name="$popoverIcon" class="w-4 h-4 opacity-40 mb-0.5" />
                                            </x-slot:trigger>
                            ...
                            <div @class(["floating-label w-full" => $label && $inline])>
                                {{-- FLOATING LABEL--}}
                                @if ($label && $inline)
                                ...
                                <div @class(["w-full", "join" => $prepend || $append])>
                                    ...
                                    >
                                        ...
                                        {{-- ICON LEFT --}}
                                        @if($icon)
                                            <x-ui-icon :name="$icon" class="pointer-events-none w-4 h-4 opacity-40" />
                                        @endif

                                        <div class="ui-choices-search flex min-w-0 flex-1 flex-wrap items-center gap-1 py-0.5">
                                            ...
                                            <span wire:key="selected-options-{{ $uuid }}">
                                                ...
                                                @else
                                                    <template x-for="(option, index) in selectedOptions" :key="index">
                                                        <span class="ui-choices-element cursor-pointer badge badge-soft m-0.5 !inline-block !h-auto">
                                                            {{-- SELECTION SLOT --}}
                                                            @if($selection)
                                                            ...
                                                            @if(!$isDisabled() && !$isReadonly())
                                                                <x-ui-icon @click="toggle(option.{{ $optionValue }})" x-show="!isReadonly && !isDisabled && !isSingle" name="o-x-mark" class="w-4 h-4 hover:text-error" />
                                                            @endif
                                                        </span>
                                                    </template>
                                                @endif
                                            ...
                                            <input
                                                ...
                                                class="ui-choices-search-input min-w-[2ch] flex-1 grow bg-transparent outline-none"
                                        ...
                                        @if($clearable && !$isReadonly() && !$isDisabled())
                                            <x-ui-icon @click="reset()" x-show="!isSelectionEmpty" name="o-x-mark" class="cursor-pointer w-4 h-4 opacity-40"/>
                                        @endif

                                        {{-- ICON RIGHT --}}
                                        @if($iconRight)
                                            <x-ui-icon :name="$iconRight" class="pointer-events-none w-4 h-4 opacity-40" />
                                        @endif
                        ...
                        {{-- OPTIONS LIST --}}
                        @php($choicesAnchor = in_array(app()->getLocale(), ['ar', 'fa', 'he', 'ur'], true) ? 'bottom-end' : 'bottom-start')
                        <div x-cloak x-show="focused" class="relative w-full" wire:key="options-list-main-{{ $uuid }}" >
                                <div
                                    wire:key="options-list-{{ $uuid }}"
                                    class="{{ $height }} absolute z-20 w-full min-w-full cursor-pointer overflow-y-auto rounded-[var(--radius-box)] border border-base-content/10 bg-base-100 shadow-lg"
                                    x-anchor.{{ $choicesAnchor }}="$refs.container"
                                >

                                   {{-- SELECT ALL --}}
                                   @if($allowAll)
                                       <div wire:key="allow-all-{{ rand() }}" class="border-b border-base-content/10 font-semibold hover:bg-base-200">
                                            <div x-show="!isAllSelected" @click="selectAll()" class="p-3 text-primary">{{ $allowAllText }}</div>
                                            <div x-show="isAllSelected" @click="reset()" class="p-3 text-error">{{ $removeAllText }}</div>
                                       </div>
                                   @endif
                                    ...
                                    <div
                                        ...
                                        class="border-b border-base-content/10 p-3 font-medium text-warning"
                                    >
                                        {{ $noResultText }}
                                    </div>
                                    ...
                                    <div x-ref="choicesOptions">
                                        @foreach($options as $option)
                                            <div
                                                ...
                                                :class="isActive({{ $getOptionValue($option) }}) && 'bg-primary/10 text-primary font-medium'"
                                                search-value="{{ data_get($option, $optionLabel) }}"
                                                class="border-b border-base-content/5 hover:bg-base-200 focus:bg-base-200 focus:outline-none"
                                                tabindex="0"
                                            >
                                                {{-- ITEM SLOT --}}
                                                @if($item)
                                                    {{ $item($option) }}
                                                @else
                                                    <x-ui-list-item :item="$option" :value="$optionLabel" :sub-value="$optionSubLabel" :avatar="$optionAvatar" />
                                                @endif
