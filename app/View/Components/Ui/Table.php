...
class Table extends Component
{
    ...
    public function render(): View|Closure|string
    {
        return <<<'HTML'
                ...
                @endisset
                <div @class([$containerClass, 'mt-8' => isset($toolbar)]) x-classes="overflow-x-auto">
                <table
                        {{
                            $attributes
                        ...
                        <thead @class(["hidden" => $noHeaders])>
                            <tr x-ref="headers">
                                ...
                                @foreach($headers as $header)
                                    ...
                                    <th
                                        class="text-xs font-semibold uppercase tracking-wide text-base-content/60 @if($isSortable($header)) cursor-pointer hover:bg-base-200 @endif {{ $header['class'] ?? ' ' }}"
                                    ...
                                    >
                                        ...
                                        @if($isSortable($header))
                                            <x-ui-icon :name="$isSortedBy($header) ? $getSort($header)['direction'] == 'asc' ? 'o-chevron-down' : 'o-chevron-up' : 'o-chevron-up-down'"  class="size-3! mb-1 ms-1" />
                                        @endif
                                    </th>
                                @endforeach
                                ...
                                <!-- ACTIONS -->
                                @if($actions)
                                    <th class="text-end text-xs font-semibold uppercase tracking-wide text-base-content/60">{{ __('common.actions') }}</th>
                                @endif
                            </tr>
                        </thead>
                    ...
                    @if($withPagination)
                        @if($perPage)
                            <x-ui-pagination :rows="$rows" :per-page-values="$perPageValues" wire:model.live="{{ $perPage }}" />
                        @else
                            <x-ui-pagination :rows="$rows" :per-page-values="$perPageValues" />
                        @endif
                    @endif
                </div>
                </div>
