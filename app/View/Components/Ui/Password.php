                            ...
                            <div
                             ...
                             >
                                ...
                                @elseif($placeToggleLeft())
                                    ...
                                    >
                                        <x-ui-icon name="{{ $passwordIcon }}" x-show="hidden" class="w-4 h-4 opacity-40" />
                                        <x-ui-icon name="{{ $passwordVisibleIcon }}" x-show="!hidden" x-cloak class="w-4 h-4 opacity-40" />
                                    </x-ui-button>
                                @endif
                                ...
                                {{-- CLEAR ICON  --}}
                                @if($clearable)
                                    <x-ui-icon x-on:click="$wire.set('{{ $modelName() }}', '', {{ json_encode($attributes->wire('model')->hasModifier('live')) }})"  name="o-x-mark" class="cursor-pointer w-4 h-4 opacity-40"/>
                                @endif

                                {{-- ICON RIGHT / TOGGLE INPUT TYPE --}}
                                @if($iconRight)
                                    <x-ui-icon :name="$iconRight" @class(["pointer-events-none w-4 h-4 opacity-40", "!end-10" => $clearable]) />
