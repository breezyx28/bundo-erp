        ...
        return <<<'HTML'
                <div
                    {{
                        $attributes
                            ->merge(['wire:key' => $uuid ])
                            ->class(['card card-border bg-base-100', 'shadow-sm' => $shadow])
                    }}
                >
                    @if($figure)
                    ...
                    @if($title || $subtitle)
                        <div class="px-4 pt-4 sm:px-5 sm:pt-5 pb-5">
                            <div class="flex gap-3 justify-between items-center w-full">
                                <div class="grow-1">
                                    @if($title)
                                        <div @class(["text-base font-semibold tracking-tight", is_string($title) ? '' : $title?->attributes->get('class') ]) >
                                            {{ $title }}
                                        </div>
                                    @endif
                                    @if($subtitle)
                    ...
                    <div @class(['grow-1 p-4 sm:p-5', $bodyClass !== 'null' ? $bodyClass : null])>
                        {{ $slot }}
                    </div>
